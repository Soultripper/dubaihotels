module HotelScopes
  ...
  module ClassMethods
    ...
    def by_location(location, proximity_in_metres = 8000)
      ...
      if location.city?
        query = query.where("ST_DWithin(hotels.geog, ?, ?) or (city= ? and country_code = ?)", location.point, proximity_in_metres, location.name.titleize, location.country_code.downcase)
        ...
        query = query.where("state_province = ?", location.name.titleize)
